---
name: review-pr
description: Use when the user says "review this PR", "/review-pr", "audit the PR without touching it", "give me a read on PR #N", "what's wrong with this PR", or asks Claude to produce a read-only audit of an open pull request without modifying code, commits, or the PR itself. Read-only mirror of /drive-pr's audit half — pulls the PR diff via gh, drives it through the slice/fan-out/merge/verify agent pipeline, and emits either a terminal report or, when explicitly requested, posts findings as PR comments via `gh pr comment`. Never edits files, never pushes, never resolves review threads, never approves or merges. Use this when you want the audit verdict on someone else's PR (or your own pre-fix); use /drive-pr when you want Claude to iterate the PR to green.
